package body Gade.Dev.Joypad is
   ...
   procedure Read_Button_Matrix (M : in out Joypad_Matrix_Type; Input : State) is
   begin
      M.P10_IN := not ((not M.P14_OUT and Input.RIGHT) or (not M.P15_OUT and Input.A));
      M.P11_IN := not ((not M.P14_OUT and Input.LEFT) or (not M.P15_OUT and Input.B));
      M.P12_IN := not ((not M.P14_OUT and Input.UP) or (not M.P15_OUT and Input.SEL));
      M.P13_IN := not ((not M.P14_OUT and Input.DOWN) or (not M.P15_OUT and Input.START));
   ...
   begin
      if Joypad.Reader /= null then
         ...
         Joypad.Map.Reg := (Joypad.Map.Reg and 16#F0#) or (New_Matrix.Reg and 16#0F#);
      end if;
   ...
   procedure Report_Cycles
     (Joypad : in out Joypad_Type; GB : in out Gade.GB.GB_Type; Cycles : M_Cycle_Count)
   is
   ...
   procedure Set_Input_Reader (Joypad : in out Joypad_Type; Reader : Reader_Access) is
   begin
