...
package Gade.Dev.CPU.Instructions.Arithmetic is
   ...
   procedure Add (CPU : in out CPU_Context; Value : Word; Result : out Word);
   --  16 Bit add

   procedure Add (CPU : in out CPU_Context; Reg : in out Word; Value : Byte);
   --  16 Bit add
   ...
   procedure Adjust_DAA (CPU : in out CPU_Context);

   generic
      Operation : Instructions.Inc_Dec_Op_Kind;
      Target : Instructions.Byte_Target_Kind;
   procedure Inc_Dec_Byte (GB : in out Gade.GB.GB_Type);

   generic
      Operation : Instructions.Inc_Dec_Op_Kind;
      Target : Instructions.Word_Register_Kind;
   procedure Inc_Dec_Word (GB : in out Gade.GB.GB_Type);

   generic
      Source : Instructions.Word_Register_Kind;
   procedure Add_HL (GB : in out Gade.GB.GB_Type);

   procedure ADD_SP_Imm8 (GB : in out Gade.GB.GB_Type);
