...
package Gade.Dev.CPU.Instructions.Loads is
   ...
   procedure LD_Byte (GB : in out Gade.GB.GB_Type);

   generic
      Dest : Instructions.Word_Register_Kind;
      Source : Instructions.Word_Source_Kind;
   procedure LD_Word (GB : in out Gade.GB.GB_Type);

   procedure LD_Addr_Imm16_SP (GB : in out Gade.GB.GB_Type);

   procedure LD_HL_SP_Plus_Imm8 (GB : in out Gade.GB.GB_Type);
