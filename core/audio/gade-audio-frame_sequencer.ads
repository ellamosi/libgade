...
private package Gade.Audio.Frame_Sequencer is
   ...
   type State is (None, Volume_Envelope, Length_Counter, Length_Counter_Frequency_Sweep);

   subtype Lengh_Step is State range Length_Counter .. Length_Counter_Frequency_Sweep;

   function Current_State (Frame_Sequencer : Audio_Frame_Sequencer) return State;
...
private
   ...
   Frame_Sequencer_Steps : constant array (Frame_Sequencer_Step_Index) of State :=
     [Length_Counter,
      None,
   ...
   procedure Step_Frame_Sequencer (Frame_Sequencer : in out Audio_Frame_Sequencer);
