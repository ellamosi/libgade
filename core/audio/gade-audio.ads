...
private package Gade.Audio is
   ...
   procedure Reset (Audio : in out Audio_Type);

   procedure Read
     (Audio : in out Audio_Type; Address : Audio_IO_Address; Value : out Byte);
...
private
   ...
   procedure Write_Power_Control_Status (Audio : in out Audio_Type; Value : Byte);

   function Frame_Sequencer_State (Audio : Audio_Type) return Frame_Sequencer.State;
