...
package body Gade.Audio.Channels is
   ...
   procedure Tick_Notify_Sample_Step is new
     Tick_Notify (Observer_Type => Audio_Channel, Notify => Step_Sample);

   procedure Create (Channel : out Audio_Channel; Audio : not null Audio_Type) is
   begin
      Channel.Audio := Audio;
   end Create;
   ...
   procedure Disable (Channel : in out Audio_Channel; Mode : Disable_Mode) is
   ...
   procedure Update_DAC_Power_State
     (Channel : in out Audio_Channel'Class; Powered : Boolean) is
   begin
      if Powered then
   ...
   procedure Next_Sample (Channel : in out Audio_Channel; Sample : out Channel_Sample) is
   begin
   ...
   begin
      ...
      Audio_Channel'Class (Channel).Next_Sample_Level (New_Sample_Level, New_Level_Time);
   ...
   procedure Write
     (Channel : in out Audio_Channel'Class; Register : Channel_Register; Value : Byte) is
   begin
      if Is_Powered (Channel.Audio) or Register = NRx1 then
         case Register is
            when NRx0 =>
               Channel.Write_NRx0 (Value);

            when NRx1 =>
               Channel.Write_NRx1 (Value);

            when NRx2 =>
               Channel.Write_NRx2 (Value);

            when NRx3 =>
               Channel.Write_NRx3 (Value);

            when NRx4 =>
               Channel.Write_NRx4 (Value);
         end case;
      end if;
   end Write;
