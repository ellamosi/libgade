...
private
   ...
   X_Flip_Lookup : constant array (Boolean'Range, Sprite_Horizontal_Range) of Natural :=
     [False => [0, 1, 2, 3, 4, 5, 6, 7], True => [7, 6, 5, 4, 3, 2, 1, 0]];

   Y_Flip_Lookup :
     constant array (Sprite_Size_Type'Range, Boolean'Range, Sprite_Vertical_Range)
     of Natural :=
       [Single =>
   ...
   Sprite_Index_Add_Lookup :
     constant array (Sprite_Size_Type'Range, Boolean'Range, Sprite_Vertical_Range)
     of Tile_Index_Type :=
       [Single => [others => [others => 0]],
        Double =>
          [False => [0 .. 7 => 0, 8 .. 15 => 1], True => [0 .. 7 => 1, 8 .. 15 => 0]]];
   ...
   procedure Insert_By_Processing_Priority
     (Buffer : in out Sprite_Priority_Buffer; Index : Sprite_Index_Type);
