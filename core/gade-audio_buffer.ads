...
package Gade.Audio_Buffer is
   ...
   type Audio_Buffer is array (Natural range <>) of Stereo_Sample with Convention => C;

   subtype Video_Frame_Buffer_Range is Natural range 0 .. Maximum_Samples - 1;

   subtype Frame_Audio_Buffer is Audio_Buffer (Video_Frame_Buffer_Range);

   type Audio_Buffer_Access is access all Frame_Audio_Buffer with Convention => C;
