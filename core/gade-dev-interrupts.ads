...
package Gade.Dev.Interrupts is
   ...
   type Interrupt_Flag_Type is new Memory_Mapped_Device with private;

   overriding
   procedure Reset (Interrupt_Flag : in out Interrupt_Flag_Type);
   ...
   type Interrupt_Enable_Type is new Memory_Mapped_Device with private;

   overriding
   procedure Reset (Interrupt_Enable : in out Interrupt_Enable_Type);
...
private
   ...
   type Interrupt_Flag_Register_Type
     (Access_Type : Interrupt_Flag_Access_Type := Named)
   is record
      case Access_Type is
         when Named =>
            VBLANK : Boolean;
   ...
   for Interrupt_Flag_Register_Type use
     record
   ...
   with Unchecked_Union;
   for Interrupt_Enable_Register_Type use
     record
       VBLANK at 0 range 0 .. 0;
       LCDC at 0 range 1 .. 1;
       Timer at 0 range 2 .. 2;
