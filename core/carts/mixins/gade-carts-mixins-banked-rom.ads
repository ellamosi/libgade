...
package Gade.Carts.Mixins.Banked.ROM is
   ...
   package Banked_ROM_Spaces is new
     Banked_Spaces
       (Banks            => Banks,
   ...
   procedure Select_ROM_Bank
     (C : in out Banked_ROM_Cart; AI : Accessible_Bank_Index; I : Bank_Index);

private
   use Address_Space_Banks, ROM_Banks;

   package ROM_Space_Carts is new
     Banked_Space_Carts (Base_Cart => Base_Cart, BS => Banked_ROM_Spaces);
   use ROM_Space_Carts;

   type Accessible_Bank_Array is array (Accessible_Bank_Index) of ROM_Bank_Access;
