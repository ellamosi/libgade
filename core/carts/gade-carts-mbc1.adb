package body Gade.Carts.MBC1 is
   ...
   overriding
   procedure Write_Special (C : in out MBC1_Cart; Value : Byte) is
   ...
   procedure Change_Banking_Mode (C : in out MBC1_Cart; New_Mode : MBC1_Banking_Mode) is
   ...
   procedure Select_Bank
     (C : in out MBC1_Cart; Address : Bank_Select_Address; Value : Byte) is
   begin
      case Address is
         when Low_Bank_Select_Address  =>
            C.Select_Low_Bank (Value);

         when High_Bank_Select_Address =>
            C.Select_High_Bank (Value);
      end case;
   end Select_Bank;

   procedure Select_Low_Bank (C : in out MBC1_Cart; Value : Byte) is
   begin
      C.Low_Bank_Select := Low_Bank_Select_Type (Value and Low_Select_Mask);
      if C.Low_Bank_Select = 0 then
         C.Low_Bank_Select := 1;
      end if;
      C.Select_ROM_Bank;
   end Select_Low_Bank;

   procedure Select_High_Bank (C : in out MBC1_Cart; Value : Byte) is
   begin
   ...
   begin
      ...
      case C.Banking_Mode is
         ...
         when Mode_1 =>
            Low_Bank_Index := Bank_Index (High_Part * 2**5);
      end case;
   ...
   begin
      case C.Banking_Mode is
         ...
         when Mode_1 =>
            Index := Bank_Index (C.High_Bank_Select);
      end case;
