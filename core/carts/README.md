...
#### MBC5 (Max 8MB ROM / 128kB RAM / Rumble bit)
Supports MBC5 ROM/RAM banking and RAM saves. Rumble cart register writes are decoded so the RAM bank bits behave correctly, but the emulator does not currently expose a rumble device.

### Unsupported
The following cartridge types are currently unsupported: MBC6, GB Camera, HuC1, HuC3, MM01, MBC1 based multicarts. Aside from some edge cases, these are all fairly rare and not a priority to implement.
