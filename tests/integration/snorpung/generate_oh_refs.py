...
from integration.snorpung.oh_sequence import OH_SAMPLES, play_oh_sequence  # noqa: E402


def main():
    parser = argparse.ArgumentParser("Generate Snorpung OH! demo reference frames")
    parser.add_argument(
        "--rom",
        default=str(TESTS_ROOT / "assets" / "roms" / "snorpung" / "oh.gb"),
        help="Path to the Snorpung OH! demo ROM",
    )
    parser.add_argument(
        "--refs-dir",
    ...
    with GadeTestdClient(executable=args.harness, timeout=args.timeout) as client:
        ...
        play_oh_sequence(client=client, rom_path=rom_path, sample_callback=save_sample)

    print("generated {} references".format(len(OH_SAMPLES)))
