...
from integration.snorpung.pocket_sequence import POCKET_SAMPLES, play_pocket_sequence  # noqa: E402


def main():
    parser = argparse.ArgumentParser("Generate Snorpung pocket demo reference frames")
    parser.add_argument(
        "--rom",
        default=str(TESTS_ROOT / "assets" / "roms" / "snorpung" / "pocket.gb"),
        help="Path to the Snorpung pocket demo ROM",
    )
    parser.add_argument(
        "--refs-dir",
    ...
    with GadeTestdClient(executable=args.harness, timeout=args.timeout) as client:
        ...
        play_pocket_sequence(client=client, rom_path=rom_path, sample_callback=save_sample)

    print("generated {} references".format(len(POCKET_SAMPLES)))
